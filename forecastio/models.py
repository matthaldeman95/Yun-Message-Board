    ...
    def _forcastio_data(self, key):
        ...
        try:
            ...
            if key == 'currently':
                ...
                return ForecastioDaily(self.json[key])
            else:
    ...
    def __unicode__(self):
        return '<ForecastioDataBlock instance: ' \
               '%s with %d ForecastioDataPoints>' % (self.summary,
                                                     len(self.data),)
class ForecastioDaily(UnicodeMixin):
    def __init__(self, d={}):
        self.d = d
        self.time = 10
        ...
        try:
            self.temp_min = int(d['data'][0]['temperatureMin'])
        except:
            self.temp_min = None

        try:
            self.temp_max = int(d['data'][0]['temperatureMax'])
        except:
            self.temp_max = None

        try:
            self.summary = d['data'][0]['summary']
        except:
            self.summary = None
