...
if __name__ == "__main__":
    with timeout(seconds=20):
        try:
            ...
            outstring = "Penguins (%s, %s): " % (rank, record)
            if int(date) == today:
                outstring += "Today,"
            elif int(date) == today + 1:
                outstring += "Tomorrow,"
            else:
                outstring += "%s %s" % (month, date)
            outstring += " %s %s %s" % (time, atvs, opponent)

            last_updated = str(datetime.datetime.now())
            data = {'last_updated': last_updated, 'data': outstring}
            with open('/mnt/sda1/data/pens.json', 'w') as outfile:
